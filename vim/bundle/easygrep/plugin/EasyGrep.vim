...
function! s:DoGrep(word, add, whole, count, escapeArgs)
    ...
    try
        if commandIsGrep && g:EasyGrepRecursive
            let grepCommand = a:count.win.com.a:add." ".opts." ".s1.word.s2." '".getcwd()."'"
        else
            let grepCommand = a:count.win.com.a:add." ".opts." ".s1.word.s2." ".filesToGrep
        endif
        echomsg "[grepCommand]".grepCommand
    ...
    if s:HasMatches()
        if g:EasyGrepOpenWindowOnMatch
            ...
            if exists("searchmanage#append")
                silent call searchmanage#append(getcwd(),a:word,getline(0,'$'),grepCommand)
            endif
            setlocal nofoldenable
        endif
    else
